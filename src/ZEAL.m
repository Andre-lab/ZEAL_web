classdef ZEAL < matlab.mixin.Copyable %handle
    ...
    methods(Access = protected)
      ...
      function cpObj = copyElement(obj)
         ...
         % Make deep copies for molshape, ZC and PDB
         cpObj.fixed.molShape = copy(obj.fixed.molShape);
         cpObj.fixed.ZC = copy(obj.fixed.ZC);
         cpObj.fixed.PDB = copy(obj.fixed.PDB);
      end
   end

    
    methods
        function obj = ZEAL(varargin)
            ...
            default_fix = ''; % = empty ZEAL object with default parameters set
            
            % Setup input parser
            p = inputParser;
            
            addOptional(p, 'fix', default_fix);
            ...
            if isempty(p.Results.fix)
                obj.InputParserOnly = true;
            end
        ...
        function updateZEAL(obj,varargin)
            
            p = inputParser();
            
            addOptional(p,'NewShape', false);
            addOptional(p,'Structure','all');
            
            parse(p, varargin{:});
            
            newshape = p.Results.NewShape;
            ZEALstructure = p.Results.Structure;
            
            % recompute moments and descriptors
            if strcmp(ZEALstructure,'all') || strcmp(ZEALstructure,'fixed')
                if newshape
                    computeShapeFunction(obj.fixed.molShape);
                end
            ...
            if strcmp(ZEALstructure,'all') || strcmp(ZEALstructure,'rotating')
                if newshape
                    computeShapeFunction(obj.rotating.molShape);
                end
            ...
            % recompute score
            obj.Search.InitialScore = computeScore(obj);
            obj.Score = obj.Search.InitialScore;
        ...
        function saveSuccessFul = save2pdb(obj,varargin)
            ...
            try
                ...
                if strcmp(selectedStructure,'rotating') ||  strcmp(selectedStructure,'all')
                    ...
                    R = getRotationMatrix(obj, 'rotating_new');
        ...
        function R = getRotationMatrix(obj, structure)
            ...
            switch structure
                ...
                case 'rotating_new'
                    ...
                    % current state
                    XYZ_new = obj.rotating.molShape.AtomData(:,1:3);
                    % original state
                    XYZ_orig = [obj.rotating.PDB.Data.X obj.rotating.PDB.Data.Y obj.rotating.PDB.Data.Z];
                    % - rotation matric from original to current state
                    R(1:3,1:3) = ZEAL.findRotation(XYZ_new',XYZ_orig');
        ...
        function obj = mergeZEAL(varargin)
            ...
            %
            % Example2
            % Create empty ZEAL object and then add a and b to it
            % c = ZEAL('');
            % ab = mergeZEAL(c,a,b)
            ...
            if numel(varargin) == 3
                mainObj = varargin{1};
                % create deep copies (ZC, PDB, molShape) otherwise we only
                % copy the handles to the new (merged) object
                fixObj = copy(varargin{2}); 
                rotObj = copy(varargin{3});
            ...
            elseif numel(varargin) == 2
                ...
                fixObj = copy(varargin{1});
                rotObj = copy(varargin{2});
                obj = fixObj;
                obj.fixed = fixObj.fixed;
                obj.rotating = rotObj.fixed;
            end
    ...
    methods (Static)
        ...
        function JSONstr = struct2JSON(PDBData, varargin)
            
            if ~isempty(varargin)
                rotating_PDBData = varargin{1};
            else
                rotating_PDBData = [];
            end
            
            fixed_PDBData = PDBData;
            
            if ~isempty(rotating_PDBData)
                JSONstr_fix = PDB.struct2JSON(fixed_PDBData,'chainID', 'A', 'modelNo',1);
                JSONstr_rot = PDB.struct2JSON(rotating_PDBData,'chainID', 'B', 'modelNo',2);
                
                JSONstr = [JSONstr_fix JSONstr_rot sprintf('END"}')];
                
            else
                JSONstr = PDB.struct2JSON(PDBData);
            end
