classdef PDB < matlab.mixin.Copyable %handle
    ...
    methods (Static)
        ...
        function saveToFile(pdbData)
            ...
            pdbData = PDB.reviewPDBdata(pdbData);
        ...
        function pdbData = reviewPDBdata(pdbData)
            ...
            pdbData.outfile = outfile;
            pdbData.recordName = recordName;
            pdbData.atomNum = atomNum;
            pdbData.altLoc = altLoc;
            pdbData.resName = resName;
            pdbData.chainID = chainID;
            pdbData.resNum = resNum;
            pdbData.occupancy = occupancy;
            pdbData.betaFactor = betaFactor;
        ...
        function JSONstr = struct2JSON(pdbData, varargin)
            ...
            addOptional(p, 'chainID', []);
            addOptional(p, 'modelNo', []);
            
            parse(p, varargin{:});
            chainID = p.Results.chainID;
            modelNo = p.Results.modelNo;
                        
            if ~isempty(chainID)
                 pdbData.chainID = cellfun(@(x) {chainID}, pdbData.chainID);
            end
